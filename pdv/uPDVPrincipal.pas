...
uses
  ...
  Vcl.WinXCtrls,
  uPDVPagamento,
  uPDVIdentificarClientes,
  uPDVImportarClientes,
  uPDVLogin,
  ZAbstractRODataset,
  ZAbstractDataset,
  ZDataset,
...
var
  frmPDVPrincipal: TfrmPDVPrincipal;

implementation

{$R *.dfm}
uses uPrincipal, uDTMVenda;

procedure TfrmPDVPrincipal.btnMaisFuncoesClick(Sender: TObject);
begin
  SplitViewAction(SplitViewFuncoes);
end;

procedure TfrmPDVPrincipal.FixarForm;
begin
...
procedure TfrmPDVPrincipal.FormCreate(Sender: TObject);
begin
...
procedure TfrmPDVPrincipal.FormDestroy(Sender: TObject);
begin
  if Assigned(oProduto) then
    FreeAndNil(oProduto);
end;

procedure TfrmPDVPrincipal.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  lPagamentos: TfrmPDVPagamento;
begin
  case Key of
    VK_ESCAPE:
    ...
    VK_F7:
      begin
        lPagamentos := TfrmPDVPagamento.Create(nil);
        lPagamentos.Parent := pnlPag;
        lPagamentos.Show;
        SplitViewAction(SplitViewPagamentos);
      end;
    VK_CONTROL: begin
      TfrmPDVImportarClientes.New(Self)
        .Embed(pnlMaster)
        .Show;
    end;
    VK_F9: begin
      TfrmPDVIdentificarClientes.New(Self)
        .IdentificaCPF
        .Embed(pnlMaster)
...
procedure TfrmPDVPrincipal.FormShow(Sender: TObject);
begin
...
procedure TfrmPDVPrincipal.MontarBotoes;
begin
...
procedure TfrmPDVPrincipal.SplitViewAction(Value: TSplitView);
begin
