...
var
  frmPDVPagamento: TfrmPDVPagamento;

implementation

{$R *.dfm}

procedure TfrmPDVPagamento.Panel21Click(Sender: TObject);
begin
  TfrmPDVCartao.New(Self).Alinhamento(alClient).Embed(pnlContainerPg);
  SetClick(ShapeCartao, pnlListaPagamentos);
end;

procedure TfrmPDVPagamento.Panel25Click(Sender: TObject);
var
  lFrame: TfrmPDVPix;
begin
  lFrame:= TfrmPDVPix.Create(nil);
...
procedure TfrmPDVPagamento.Panel29Click(Sender: TObject);
var
  lFrame: TfrmPDVDinheiro;
begin
  lFrame:= TfrmPDVDinheiro.Create(Self);
  lFrame.Align := alClient;
  lFrame.Parent := pnlContainerPg;
...
procedure TfrmPDVPagamento.SetClick(Shape: TShape; Panel: TPanel);
begin
