...
{$REGION 'Verificação e Criação de Tabelas do Banco de Dados'}
...
procedure TAtualizacaoTableMSSQL.StatusCliente;
begin
  if not TabelaExiste('statuscliente') then
  begin
    ExecutaDiretoBancoDeDados(
	   'CREATE TABLE statuscliente( '+
     '      statusclienteId int IDENTITY(1,1) NOT NULL, '+
	   '      descricao  varchar(30) NULL, '+
	   '	 PRIMARY KEY (statusclienteId)) ');
    ExecutaDiretoBancoDeDados('INSERT INTO statuscliente(descricao) VALUES (''Ativo''), '+
                              '(''Bloqueado''), (''Atenção''), (''Inativo''), (''Prospecto'')');
  end;
end;
...
begin
  ...
  begin
    ...
    ExecutaDiretoBancoDeDados('INSERT INTO pessoa(descricao) VALUES (''Física''), '+
                              '(''Jurídica'')');
  end;
end;
