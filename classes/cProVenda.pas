...
begin
  if MessageDlg('Apagar o Registro: '+#13+#13+
                'Venda Número: '+IntToStr(F_VendaId),mtConfirmation,[mbYes, mbNo],0)=mrNo then begin
     Result:=false;
     abort;
  end;
...
begin
  try
    ...
    Except
      ...
      ConexaoDB.Rollback;
      Result:=false;
       MessageDlg('Erro na Atualizacao: '+e.message, mtError, [mbOK] ,0);
      end;
    End;
...
begin
    ...
    Try
      ...
      try
        ...
        on e: exception do
        begin
           MessageDlg('Erro ao Retornar Estoque: '+e.message, mtError, [mbOK] ,0);
        end;
      end;
    Finally
...
begin
  try
    ...
    Qry.SQL.Add(' DELETE '+
                ...
                '  WHERE VendaId=:VendaId ');
   if sCodNoCds<>'' then
    Qry.SQL.Add('    AND produtoId NOT IN ('+sCodNoCds+') ');
