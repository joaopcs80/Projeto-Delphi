...
begin
  try
    ...
    begin
      if (aForm.Components[i] is TPngSpeedButton) then
      begin
        if TPngSpeedButton(aForm.Components[i]).Tag=99 then
        begin
          oAcaoAcesso.descricao := '    - BOTÃO '+ StringReplace(TPngSpeedButton(aForm.Components[i]).Hint, '&','',[rfReplaceAll]);
          oAcaoAcesso.Chave     := aForm.Name+'_'+TPngSpeedButton(aForm.Components[i]).Name;
          if not oAcaoAcesso.ChaveExiste(oAcaoAcesso.Chave) then
             oAcaoAcesso.Inserir;
        end;
      end;
